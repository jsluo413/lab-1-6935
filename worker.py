    ...
    try:
        with Client(MANAGER_ADDRESS, authkey=AUTH_KEY) as conn:
            ...
            if response.get('status') == 'SUCCESS':
                print(
                    f"WORKER: Successfully registered with manager at {MANAGER_ADDRESS}")
            else:
                print(
                    f"WORKER: Registration failed: {response.get('message')}")
                sys.exit(1)
    except Exception as e:
        print(
            f"WORKER: Could not connect to manager at {MANAGER_ADDRESS}: {e}")
        sys.exit(1)
